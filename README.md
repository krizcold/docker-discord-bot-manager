...
- **From the UI** (standalone docker mode): the header shows the commit the running image was built from (checked in the background, cached for 60s) and an **Update manager** button when the repo is behind its upstream or the running build is behind the checkout (the badge then reads "build behind the checkout", as after a `git pull` by hand with no rebuild). It streams the pull + build; when nothing was pulled and the rebuilt image is unchanged it reports already up to date without restarting, otherwise it recreates the manager and verifies the restart actually took effect; the page reconnects and shows the new version. (Hidden on Yundera, where the platform handles updates. On Windows the rebuild always works; if the auto-restart can't launch, the UI tells you the one-line command to finish it.)

...
export interface TransferOptions {
  ...
  /** The operator has seen that other instances cannot reach this one once it is the primary side. */
...
export async function transferSide(instance: InstanceConfig, opts: TransferOptions): Promise<ActionResult> {
  ...
  if (instance.fleetDbReplica) {
    ...
    if (!active || active.phase === 'promoting' || opts.confirmLag === true || opts.confirmLineage === true || opts.confirmReachability === true) {
      const result = await promoteSide(instance, { confirmLag: opts.confirmLag, confirmLineage: opts.confirmLineage, confirmReachability: opts.confirmReachability, retireOldMaster: opts.retireOldMaster });
...
async function runSeedFirst(botId: string, startedAt: number): Promise<void> {
  ...
  try {
    ...
    if (!result.success) {
      ...
      // Each of the app's acknowledgements is answered the same way, by the
      // operator continuing the run from the panel with the confirm attached.
      throw new Error(result.needsLagConfirm || result.needsLineageConfirm || result.needsReachabilityConfirm
        ? `${result.error || (result.needsLagConfirm ? 'the app wants the replication lag acknowledged' : result.needsLineageConfirm ? 'another designated backup received further than this copy' : 'the app wants it acknowledged that other instances cannot reach this one')}; the standby now exists, so Continue transfer on the standby panel answers it`
